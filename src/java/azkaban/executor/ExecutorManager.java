...
public class ExecutorManager extends EventHandler implements ExecutorManagerAdapter {
    ...
    public String submitExecutableFlow(ExecutableFlow exflow, String userId) throws ExecutorManagerException {
        synchronized (exflow) {
            ...
            if (null != options.getRerunExecid() && !"".equals(options.getRerunExecid().trim())) { //zhongshu-comment 如果是重跑，就只更新记录的一些字段吧，问题是更新哪些字段呢？question
				...
				/*
                zhongshu-comment 这里才是真正的submitTime
                第一次执行execution时，会使用给这个新生成的submitTime，
                重跑任务时就不会生成新的submitTime了
                 */
                //zhongshu-comment question 如果是重跑任务时不新增一个execution，那么这个原纪录的submit_time字段值不应该被修改，所以是update时不会update该字段
    ...
    private Map<String, Object> callExecutorServer(String host, int port, String action, Integer executionId, String user, Pair<String, String>... params) throws IOException {
        ...
        String error = (String) jsonResponse.get(ConnectorParams.RESPONSE_ERROR);//zhongshu-comment
        if (error != null) {
